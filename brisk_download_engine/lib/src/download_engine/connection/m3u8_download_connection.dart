...
class M3U8DownloadConnection extends HttpDownloadConnection {
  ...
  bool get isDownloadCompleted {
    final finalSegmentPath = join(tempDirectory.path, "final-segment.ts");
  ...
  init(connectionReset, progressCallback, _) async {
    await terminateConnection();
    connectionStatus = connectionReset
  ...
  Future<void> initClient(bool connectionReset) async {
    client?.close();
    client = ScopedMediaClient(
      await HttpClientBuilder.buildClient(
        HttpClientSettings(
          clientType: ClientType.dartHttp,
          proxySetting: settings.clientSettings?.proxySetting,
        ),
      ),
    );
  }
